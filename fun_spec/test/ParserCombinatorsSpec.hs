...
spec = do
  describe "eof" $ do
    it "succeeds on empty input" $
      runStateT (eof :: Parser Int ()) [] `shouldBe` Ok ((), [])
    it "fails on non-empty input" $
      runStateT (eof :: Parser Int ()) [1] `shouldBe` Err "expected eof but found: [1]"

  describe "satisfy" $ do
    it "succeeds when predicate is met" $ do
      let p = satisfy (\x -> if x > 0 then Just x else Nothing)
      runStateT (p :: Parser Int Int) [1, 2] `shouldBe` Ok (1, [2])
    it "fails when predicate is not met" $ do
      let p = satisfy (\x -> if x > 0 then Just x else Nothing)
      runStateT (p :: Parser Int Int) [-1, 2] `shouldBe` Err "failed to satisfy predicate at [-1,2]"

  describe "token" $ do
    it "consumes the correct token" $
      runStateT (token '1') ['1', '2'] `shouldBe` Ok ('1', ['2'])
    it "fails on incorrect token" $
      runStateT (token '1') ['2', '1'] `shouldBe` Err "expected '1', found \"21\""
  ...
  describe "<|>" $ do
    ...
    it "chooses the second parser if the first fails" $ do
      let p = token 1 <|> token 2
      runStateT (p :: Parser Int Int) [2, 1] `shouldBe` Ok (2, [1])
  ...
  describe "alt" $ do
    ...
    it "returns Left if the first parser succeeds" $
      runStateT p [1, 2] `shouldBe` Ok (Left 1, [2])
    it "returns Right if the first parser fails and second succeeds" $
      runStateT p [10, 1] `shouldBe` Ok (Right "10", [1])

  describe "oneof" $ do
    it "picks the first successful parser" $ do
      let p = oneof [token 1, token 2, token 3]
      runStateT (p :: Parser Int Int) [2, 3] `shouldBe` Ok (2, [3])
    it "fails if all parsers fail" $ do
      let p = oneof [token 1, token 2]
      runStateT (p :: Parser Int Int) [3, 4] `shouldBe` Err "no rule matched"

  describe "opt" $ do
    it "returns Just the value if parser succeeds" $
      runStateT (opt (token '1')) ['1', '2'] `shouldBe` Ok (Just '1', ['2'])
    it "returns Nothing if parser fails" $
      runStateT (opt (token '1')) ['2', '1'] `shouldBe` Ok (Nothing, ['2', '1'])

  describe "rpt" $ do
    it "parses zero or more occurrences" $ do
      runStateT (rpt (token '1')) ['1', '1', '2'] `shouldBe` Ok (['1', '1'], ['2'])
      runStateT (rpt (token '1')) ['2', '1', '1'] `shouldBe` Ok ([], ['2', '1', '1'])
    it "works with alternatives" $
      runStateT (rpt (token '1' <|> token '2')) ['2', '1', '3'] `shouldBe` Ok (['2', '1'], ['3'])

  describe "rptSep" $ do
    it "parses zero or more occurrences with a separator" $ do
      let p = rptSep (token '1') (token '0')
      runStateT p ['1', '0', '1', '2'] `shouldBe` Ok (Just ('1', [('0', '1')]), ['2'])
      runStateT p ['2', '1', '0'] `shouldBe` Ok (Nothing, ['2', '1', '0'])
    it "works with alternatives" $ do
      let p = rptSep (token '1' <|> token '2') (token '3' <|> token '4')
      runStateT p ['2', '3', '1', '4', '2'] `shouldBe` Ok (Just ('2', [('3', '1'), ('4', '2')]), [])

  describe "rptDropSep" $ do
    it "parses zero or more occurrences and drops the separator" $ do
      let p = rptDropSep (token '1') (token '0')
      runStateT p ['1', '0', '1', '2'] `shouldBe` Ok (['1', '1'], ['2'])
      runStateT p ['2', '1', '0'] `shouldBe` Ok ([], ['2', '1', '0'])
    it "works with alternatives" $ do
      let p = rptDropSep (token '1' <|> token '2') (token '3' <|> token '4')
      runStateT p ['2', '3', '1', '4', '2'] `shouldBe` Ok (['2', '1', '2'], [])

  describe "some" $ do
    it "parses one or more occurrences" $
      runStateT (some (token '1')) ['1', '1', '2'] `shouldBe` Ok (['1', '1'], ['2'])
    it "fails if no occurrences found" $
      case runStateT (some (token '1')) ['2', '1', '1'] of
        Err _ -> return ()
        Ok _ -> expectationFailure "Expected failure but got success"
  ...
  describe "between" $ do
    it "parses content between delimiters" $ do
      ...
      runStateT p ['(', 'x', ')', 'y'] `shouldBe` Ok ('x', ['y'])
  ...
  describe "chainl1" $ do
    ...
    it "parses left-associative expressions" $
      runStateT p ['1', '+', '2', '+', '3'] `shouldBe` Ok (6, [])
    it "handles single value without operators" $
      runStateT p ['5'] `shouldBe` Ok (5, [])
  ...
  describe "chainr1" $ do
    ...
    it "parses right-associative expressions" $
      ...
      runStateT p ['5', '-', '3', '-', '1'] `shouldBe` Ok (3, [])
    it "handles single value without operators" $
      runStateT p ['5'] `shouldBe` Ok (5, [])
  ...
  describe "tokens" $ do
    it "matches a sequence of tokens" $ do
      let p = tokens ['a', 'b', 'c']
      runStateT p ['a', 'b', 'c', 'd'] `shouldBe` Ok (['a', 'b', 'c'], ['d'])
    ...
    it "succeeds on empty sequence" $ do
      let p = tokens ""
      runStateT p ['a', 'b'] `shouldBe` Ok ([], ['a', 'b'])

  describe "string" $ do
    it "matches a string" $ do
      let p = string "hello"
      runStateT p "hello world" `shouldBe` Ok ("hello", " world")
